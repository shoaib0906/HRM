...
Class HolidayController extends Controller{
	...
	public function index(Holiday $holiday){
        ...
        foreach($holidays as $holiday){
			...
			$cols = array('<div class="btn-group btn-group-xs">'.
					...
					'<a href="/holiday/'.$holiday->id.'/Delete" class="btn btn-default btn-xs md-trigger">  <i class="fa fa-trash"></i></a> '.
					'</div>',
	...
	public function edit(Holiday $holiday){
		...
		if(!Entrust::can('edit_holiday'))
			return redirect('/dashboard')->withErrors(config('constants.NA'));
		$holiday->date = date('m/d/Y', strtotime($holiday->date));
	...
	public function update(HolidayRequest $request,Holiday $holiday){
	    ...
	    $data = $request->all();
		$holiday->fill($data);
		$holiday->date = date('Y-m-d', strtotime($request->input('date')));
		$holiday->save();
	...
	public function destroy($id,Holiday $holiday){
