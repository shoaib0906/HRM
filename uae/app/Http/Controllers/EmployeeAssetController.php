...
Class EmployeeAssetController extends Controller{
	...
	public function index(EmployeeAsset $employeeasset){
        ...
        foreach($employeeassets as $employeeasset){
			...
			$cols = array('<div class="btn-group btn-group-xs">'.
					...
					'<a href="/employeeasset/'.$employeeasset->id.'/delete" class="btn btn-default btn-xs md-trigger"> <i class="fa fa-trash"></i></a> '.
					'</div>',
					$name,
	...
	public function edit(EmployeeAsset $employeeasset){
		...
		$employeeasset->issue_date = date('m/d/Y',strtotime($employeeasset->employeeasset));
	...
	public function store(EmployeeAssetRequest $request,EmployeeAsset $employeeasset){	
		...
		$employeeasset->issue_date = date('Y-m-d',strtotime($request->input('issue_date')));
	...
	public function update(EmployeeAssetRequest $request,EmployeeAsset $employeeasset){
		...
		$employeeasset->issue_date = date('Y-m-d',strtotime($request->input('issue_date')));
		$employeeasset->return_date = date('Y-m-d',strtotime($request->input('return_date')));
		$employeeasset->save();
	...
	public function destroy($id,EmployeeAsset $employeeasset){
        ...
        $employeeasset = EmployeeAsset::find($id);
        $employeeasset->delete();
        $activity = 'Deleted a Employee Asset';
		Activity::log($activity);
