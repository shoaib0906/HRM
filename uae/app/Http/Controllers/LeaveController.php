...
Class LeaveController extends Controller{
	...
	public function index(Leave $leave){
        ...
        foreach($leaves as $leave){
			...
			$cols = array(
					...
					'<a href="/leave/'.$leave->id.'" class="btn btn-default btn-xs" data-toggle="tooltip" title="View"> <i class="fa fa-eye"></i></a> '.
					'<a href="/leave/'.$leave->id.'/edit" class="btn btn-default btn-xs" data-toggle="tooltip" title="Edit"> <i class="fa fa-edit"></i></a> '.
					'<a href="/leave/'.$leave->id.'/delete" class="btn btn-default" data-toggle="tooltip" title="Delete"> <i class="fa fa-trash"></i></a></div>',
	...
	public function edit(Leave $leave){
        ...
        $users= $query->join('designations','designations.id','=','users.designation_id')
            ...
            ->select(DB::raw('CONCAT(first_name, " ", last_name, " (", designation, " in ", department_name, " Department)") AS full_name,users.id AS user_id'))
            ->lists('full_name','user_id');
        	    $leave->from_date = date('m/d/Y', strtotime( $leave->from_date));
	    $leave->to_date = date('m/d/Y', strtotime( $leave->to_date));
	...
	public function store(LeaveRequest $request, Leave $leave){	
		...
		$data = $request->all();
	    $leave->fill($data);
	    $leave->from_date = date('Y-m-d', strtotime( $from_date));
	    $leave->to_date = date('Y-m-d', strtotime( $to_date));
