...
Class LocationController extends Controller{
	...
	public function index(Location $location){
        ...
        foreach($locations as $location){
			...
			$cols = array(
				...
				'<a href="/location/'.$location->id.'/edit"  data-name="" class="delete_all btn btn-xs btn-default"  title="Delete"> <i class="fa fa-trash"></i></a> '.
				'</div>',
	...
	public function destroy($id,Location $location){
        ...
        if(!Helper::getMode())
            return redirect()->back()->withErrors(config('constants.DISABLE_MESSAGE'));
		$emploc = DB::select( DB::raw("SELECT location_id FROM users WHERE location_id = ".(int)$id));
