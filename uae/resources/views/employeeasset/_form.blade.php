              ...
              <div class="form-group">
                ...
                {!! Form::select('employee_id', [null=>''] + $users,isset($employeeasset->employee_id) ? $employeeasset->employee_id : '',['class'=>'form-control  select2','id'=>'select21','placeholder'=>'Select Employee'])!!}
              </div>	
              <div class="form-group">
                {!! Form::label('asset_id',trans('messages.Asset'),['class' => 'col-sm-2 control-label'])!!}                
                {!! Form::select('asset_id', [null=>'Please Select'] + $asset_types,isset($employeeasset->asset_id) ? $employeeasset->asset_id : '',['class'=>'form-control input-xlarge select2','id'=>'select22','placeholder'=>'Select Asset'])!!}
              </div>	
			  <div class="form-group">              	
			    {!! Form::label('issue_date',trans('messages.Issue Date'),[])!!}                
                {!! Form::input('text','issue_date',isset($employeeasset->issue_date) ? $employeeasset->issue_date : '',['class'=>'form-control','id'=>'rangepicker4','placeholder'=>'Enter Issue Date','readonly' => 'true'])!!}
			  </div>
              @if(isset($employeeasset->id))
              <div class="form-group">              	
			    {!! Form::label('return_date',trans('messages.Return Date'),[])!!}                
                {!! Form::input('date','return_date',isset($employeeasset->return_date) ? $employeeasset->return_date : '',['class'=>'form-control','placeholder'=>'Enter Return Date'])!!}
			  </div>
              @endif
			  <div class="form-group">
